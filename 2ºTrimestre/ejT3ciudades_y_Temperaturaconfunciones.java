...
public class ejT3ciudades_y_Temperaturaconfunciones {
    ...
    // funcion para ver si se repite ciudad
    public static boolean repetir(ArrayList<String> ciudad, String F) {
        boolean m = false;
        for (int i = 0; i < ciudad.size(); i++) {
            if (ciudad.contains(F) == true)
                m = ciudad.contains(F);
    ...
    // funcion para rellenar las arraylist de las ciudades con mayor o menor
    // temperatura
    public static ArrayList<String> rellenar(ArrayList<String> ciu_lista, Double T, String F, Double grados) {

        grados = (T > grados) ? T : grados;
        if (T == grados)
            ciu_lista.add(F);
        return ciu_lista;
    }
