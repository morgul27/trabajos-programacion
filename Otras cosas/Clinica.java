...
public class Clinica {
    ...
    public static void main(String[] args) {
        ...
        //cada visita tiene información almacendada en 7 enteros que corresponden a los distintos datos
...
//* se puede incluir un incremento de 3 si es viernes, previo posicionamiento inicial en 1 dia laborable
            dia=dia1.plusDays(i);
            diaValido=true;
            ...
            if (dia.getDayOfWeek()==DayOfWeek.SATURDAY ||
                ...
                diaValido=false;
            if (festivos.contains(dia)) diaValido=false;
    ...
    public static void genVisitasDia(ArrayList<int[]> v, String especialistas[],int[][] ramasE, int nDia, String fCobro[], LocalDate dia1) {
 //       LocalDate dia1 = LocalDate.of(2022, Month.MARCH, 1);
        ...
        for(int i=0;i<especialistas.length ;i++){
			//exclusion de dias de las semana libres. Se podria/deberia mejorar con un un arrylist
            if (!(
                (especialistas[i].equalsIgnoreCase("juan") && dia.getDayOfWeek()==DayOfWeek.MONDAY ) ||
                (especialistas[i].equalsIgnoreCase("maria") && dia.getDayOfWeek()==DayOfWeek.FRIDAY )
                )){

			//se toma la especialidad que corresponde al especialista o -1 para indicar que puede ser cualquiera de las especialidades
			//es una opción "rapida".Mejorable tratando como aleatoria la posicion de ramasE[i] cuando fuera de longitud >1
            if(ramasE[i].length==1) especialidad=ramasE[i][0]; else especialidad=-1;
    ...
    //en el parametro rama se indica el indice de la especialidad o si es -1 para aleatorio
...
//para sustituir el 2 por la longitud-1 del array de fcobros, habria que arrastrarlo en el envio de parametros o declar el array de forma de cobro como "global"
        if (iRama==-1)  ramaV=aleatorio(0,1);else ramaV=iRama;
        //visita = new int[7];
        //se declara, crea e iniciaiza de una sola vez, el array con los datos de cada visita
...
//*****************************************************************************
    ...
    public static void listarVisitas(ArrayList<int[]> v, String Especialistas[], String Ramas[], String fCobro[]) {
        ...
        for(int i=0; i<v.size();i++){
                ...
                pac=visita[3];
                if (visita[4]==1) urgencia="** URGENCIA **"; else urgencia=" ";
                precio=visita[5];
